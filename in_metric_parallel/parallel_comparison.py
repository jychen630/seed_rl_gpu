...
SHOW_ALGORITHMS = {
    'Thompson': True,     # Thompson
    'SeedSampling': True,  # SeedSampling
    'UCRL': True,        # UCRL
    'Seed TD': True,      # Seed TD
    'Seed LSVI': True     # Seed LSVI
}
...
output_dir = Path('parallel_plots')
...
def create_plot(metric_name, data, ylabel):
    ...
    plt.title(f'Parallel Chain: {ylabel} vs Number of Agents')
...
# Additional comparisons across implementations
def create_comparative_plots():
    # Comparative performance plot for time across all implementations
    for metric_name, ylabel in metric_labels.items():
        data = read_metric_data(metric_name)
        if data is None:
            continue
        ...
        # Use a different subplot for each algorithm
        fig, axes = plt.subplots(len(algorithms), 1, figsize=(12, 5*len(algorithms)), sharex=True)
        
        for i, algo in enumerate(algorithms):
            algo_data = data[data['Algorithm'] == algo]
            
            for impl in algo_data['Implementation'].unique():
                impl_data = algo_data[algo_data['Implementation'] == impl]
                impl_type = get_implementation_type(impl)
            ...
            axes[i].set_title(f'{algo} - {ylabel}')
        ...
        plt.xscale('log')
        plt.xticks(k_values, [str(k) for k in k_values])
        axes[-1].set_xlabel('Number of Concurrent Agents (K)')
        
        plt.tight_layout()
        plt.savefig(output_dir / f'{metric_name}_by_algorithm.png', bbox_inches='tight', dpi=300)
        plt.close()
        
        # Create a comparison of implementations for each K value
        for k in k_values:
            k_col = f'K={k}'
            plt.figure(figsize=(12, 8))
            
            # Organize data by implementation and algorithm
            implementations = sorted(data['Implementation'].unique())
            algo_data = {}
            
            for algo in algorithms:
                algo_values = []
                for impl in implementations:
                    impl_rows = data[(data['Algorithm'] == algo) & (data['Implementation'] == impl)]
                    if not impl_rows.empty and k_col in impl_rows.columns:
                        value = impl_rows[k_col].values[0]
                        if metric_name == 'throughput_per_agent':
                            value = 1/value if value != 0 else np.nan
                        algo_data.setdefault(algo, []).append(value)
                    else:
                        algo_data.setdefault(algo, []).append(np.nan)
            
            # Create bar chart
            x = np.arange(len(implementations))
            width = 0.8 / len(algorithms)
            
            for i, (algo, values) in enumerate(algo_data.items()):
                plt.bar(x + i*width - 0.4 + width/2, values, width, label=algo)
            
            plt.xlabel('Implementation')
            plt.ylabel(ylabel)
            plt.title(f'Parallel Chain: {ylabel} for K={k}')
            ...
            plt.tight_layout()
            plt.savefig(output_dir / f'{metric_name}_K{k}_comparison.png', bbox_inches='tight', dpi=300)
            plt.close()

# Run the additional comparative plots
