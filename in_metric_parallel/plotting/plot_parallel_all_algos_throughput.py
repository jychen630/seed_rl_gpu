...
# Function to read and combine data for throughput metric
def read_throughput_data():
    all_data = []
    
    for dir_name, label in directories.items():
        file_name = f"../{dir_name}/{dir_name}_time_per_agent.csv"
        try:
...
# Function to create subplots for throughput
def create_throughput_subplots(data):
    ...
    # Manually set the winners for each algorithm for throughput
    # For throughput, higher is better (opposite of time_per_agent)
    best_frameworks = {
        'SeedSampling': ['NumPy'],
        'Seed TD': ['Torch'],
        'Seed LSVI': ['Torch'],
    ...
    for i, algo in enumerate(ALGORITHMS):
        ...
        # Add boxes for best framework(s) (higher is better, so place boxes at bottom)
        if algo in best_frameworks and best_frameworks[algo]:
            ...
            for idx, best_framework in enumerate(best_frameworks_list):
                ...
                ax.text(
                    0.95, 0.95 + vertical_offset,  # Bottom right placement
                    best_framework, 
                    ...
                    ha='right', 
                    va='top',  # Align to bottom for throughput
                    fontsize=12,
                    fontweight='bold',
                    color='white'
                )
    ...
    plt.suptitle('Parallel Environment: Throughput (agents/ms) vs Number of Agents (K)', fontsize=16)
    ...
    fig.text(0.98, 0.98, "↑ HIGHER IS BETTER",  # Changed direction indicator
             bbox=dict(facecolor='white', alpha=0.9, edgecolor='black', boxstyle='round,pad=1'),
             ha='right', va='top',
             fontsize=14, fontweight='bold')
    
    # Save the plot
    plt.savefig(output_dir / 'parallel_all_algos_throughput.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
