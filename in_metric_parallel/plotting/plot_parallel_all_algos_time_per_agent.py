...
# Function to read and combine data for time_per_agent metric
def read_time_data():
    all_data = []
...
# Function to create subplots for time_per_agent
def create_time_subplots(data):
    ...
    # Manually set the winners for each algorithm for time_per_agent
    best_frameworks = {
        'SeedSampling': ['NumPy'],
        'Seed TD': ['Torch'],
        'Seed LSVI': ['Torch'],
        'UCRL': ['NumPy', 'Torch'],
        'Thompson': ['NumPy', 'Torch']
    }
    ...
    plt.suptitle('Parallel Environment: Time per Agent (ms) vs Number of Agents (K)', fontsize=16)
    ...
    # Save the plot
    plt.savefig(output_dir / 'parallel_all_algos_time_per_agent.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
