...
# Function to read and combine data for gpu_per_agent metric
def read_gpu_data():
    all_data = []
    
    for dir_name, label in directories.items():
        file_name = f"../{dir_name}/{dir_name}_gpu_per_agent.csv"
        try:
...
# Function to create subplots for gpu_per_agent
def create_gpu_subplots(data):
    ...
    # Manually set the winners for each algorithm for gpu_per_agent
    # NumPy doesn't use GPU, so it's always the best for GPU usage
    best_frameworks = {
        'SeedSampling': ['NumPy', 'JAX', 'Torch'],
        'Seed TD': ['NumPy', 'JAX', 'Torch'],
        'Seed LSVI': ['NumPy', 'JAX', 'Torch'],
        'UCRL': ['NumPy', 'JAX', 'Torch'],
        'Thompson': ['NumPy', 'JAX', 'Torch']
    }
    ...
    plt.suptitle('Parallel Environment: GPU Memory per Agent (MB) vs Number of Agents (K)', fontsize=16)
    ...
    # Save the plot
    plt.savefig(output_dir / 'parallel_all_algos_gpu_per_agent.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
