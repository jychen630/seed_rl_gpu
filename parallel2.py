...
def run_algorithm(K, repeats, algo, env, mu0, sigma0_sq, Sigma0, O):
    regrets = []
    for _ in tqdm(range(repeats), desc=f"{algo}"):
        ...
        for _ in range(K):
            ...
            if algo == 'thompson':
                ...
                reward = env.observe_reward(chosen_chain)
            elif algo == 'standard':
        ...
        regret = np.max(theta_star) - np.mean(rewards)
...
for algo in results:
    for K in tqdm(num_agents_list, desc=f"{algo} * num agents: "):
        avg_regret = run_algorithm(K, repeats, algo, env, mu0, sigma0_sq, Sigma0, O)
...
for algo in cumulative_rewards:
    np.random.seed(42)
    chain_rewards = []
    for _ in range(K_large):
        if algo == 'ucrl':
            chain = np.argmax(sigma0_sq)
        else:
            chain = np.random.choice(C)
        r = env.observe_reward(chain)
        chain_rewards.append(r)
    cumulative_regret = np.cumsum(np.max(env.theta) - np.array(chain_rewards))
    cumulative_rewards[algo] = cumulative_regret

plt.figure(figsize=(8, 5))
for algo in ['thompson', 'standard', 'martingalean', 'ucrl']:
    plt.plot(range(K_large), cumulative_rewards[algo], label=algo)
plt.xlabel('Agent Index (sorted by start time)')
plt.ylabel('Cumulative Regret')
plt.title('Figure 4b: Cumulative Regret of 100,000 Agents')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("parallel2b.png")
