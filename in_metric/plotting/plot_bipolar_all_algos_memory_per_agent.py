...
def read_memory_data():
    ...
    for dir_name, label in directories.items():
        file_name = f"../{dir_name}/{dir_name}_memory.csv"
        try:
...
def create_memory_subplots(data):
    ...
    plt.suptitle('Bipolar Chain: Memory per Agent (MB) vs Number of Agents (K)', fontsize=16)
    ...
    # Save the plot
    plt.savefig(output_dir / 'bipolar_all_algos_memory_per_agent.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
