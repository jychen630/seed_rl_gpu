...
def read_gpu_data():
    ...
    for dir_name, label in directories.items():
        file_name = f"../{dir_name}/{dir_name}_gpu.csv"
        try:
...
def create_gpu_subplots(data):
    ...
    # For GPU, lower usage is better, but NumPy doesn't use GPU at all
    best_frameworks = {
        'SeedSampling': ['JAX', 'NumPy', 'Torch'],
        'Seed TD': ['JAX', 'NumPy', 'Torch'],
        'Seed LSVI': ['NumPy', 'Torch'],
        'UCRL': ['JAX', 'NumPy', 'Torch'],
        'Thompson': ['JAX', 'NumPy', 'Torch']
    }
    ...
    plt.suptitle('Bipolar Chain: GPU Memory per Agent (MB) vs Number of Agents (K)', fontsize=16)
    ...
    # Save the plot
    plt.savefig(output_dir / 'bipolar_all_algos_gpu_per_agent.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
