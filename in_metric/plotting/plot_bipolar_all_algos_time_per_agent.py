...
def create_time_subplots(data):
    ...
    # Manually set the winners for each algorithm as specified
    best_frameworks = {
        'SeedSampling': ['NumPy', 'JAX'],
        'Seed TD': ['Torch'],
        'Seed LSVI': ['Torch'],
        'UCRL': ['JAX', 'NumPy'],
        'Thompson': ['NumPy']
    ...
    plt.suptitle('Bipolar Chain: Time per Agent (ms) vs Number of Agents (K)', fontsize=16)
    ...
    # Save the plot
    plt.savefig(output_dir / 'bipolar_all_algos_time_per_agent.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
