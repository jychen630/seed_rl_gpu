...
def simulate_bipolar_chain(K, N, H, true_theta_L, true_theta_R, algorithm, n_sims=50):
    optimal_reward = (N // 2 + 1) * H  # R* = N/2 +1 (paper's definition)
    regrets = []
    ...
    for _ in range(n_sims):
        ...
        for agent in agents:
            # Agent's path (starts at midpoint)
            path = [env.mid]
...
N = 100  # Odd number (vertices 0-99)
H = 150  # Horizon
true_theta_L, true_theta_R = N, -N  # Left is optimal (R* = N/2 +1)
...
#plt.ylim(0, 200)  # Cap y-axis at 200
...
plt.savefig("bipolar_chain_regret.png")