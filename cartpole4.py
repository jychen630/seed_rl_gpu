...
class SeedTDNeuralAgent:
    ...
    def step_once(self, env, replay_buffer, timestep_rewards, step):
        obs_tensor = torch.tensor(featurize(env.state)).float().unsqueeze(0).to(self.device)
        with torch.no_grad():
            q_vals = self.q(obs_tensor) + 3.0 * self.q_prior(obs_tensor)
            action = torch.argmax(q_vals).item()
        step_out = env.step(action)
        noise = self._next_noise()
        replay_buffer.append((env.state, action, step_out.reward + noise, step_out.new_obs))
...
def train_with_traj(K=100, seed=0):
    ...
    shared_buffer = []
    agents = [SeedTDNeuralAgent(device=device) for _ in range(K)]
    envs = [CartPole() for _ in range(K)]
    # Make sure to initialize env.state before step_once
    for env in envs:
        env.reset()
    ...
    for step in range(TIMESTEP):
        for k in range(K):
            if not dones[k]:
                dones[k] = not agents[k].step_once(envs[k], shared_buffer, timestep_rewards, step)
...
def run_figure3_combined():
    ...
    for K in K_values:
        ...
        reward_traj += train_with_traj(K, seed=K)
