...
class Agent:
    def __init__(self, agent_id, algorithm, env):
        ...
        self.env = env
        # Initialize chosen_chain for all algorithms
        if self.algorithm == "Thompson":
            ...
            # For SeedSampling, we'll compute chosen_chain in choose_chain()
    ...
    def choose_chain(self):
        if self.algorithm == "Thompson":
            ...
            # SeedSampling implementation
            self.seed_theta0 = np.random.normal(0, np.sqrt(2 * self.env.prior_vars))
            ...
            self.chosen_chain = np.argmax(theta_hat)
            return self.chosen_chain
...
def simulate_parallel_chains(K, C, H, prior_vars, algorithm, n_sims=50):
    ...
    for _ in range(n_sims):
        ...
        for k in range(K):
            ...
            # Commit to the chosen chain for H steps
            for h in range(H):
                reward = env.observe_chain(chosen_chain)
                total_reward += reward

            regret = (env.optimal_reward * H) - total_reward
...
for alg in algorithms:
    ...
    for K in tqdm(K_values):
        avg_regret, _ = simulate_parallel_chains(K, C, H, prior_vars, alg, n_sims=30)
...
plt.legend()
plt.tight_layout()
plt.savefig("parallel_corrected.png")
