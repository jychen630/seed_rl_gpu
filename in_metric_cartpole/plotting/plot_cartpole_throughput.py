...
# Function to create subplot for throughput_per_agent
def create_throughput_subplot(torch_data, numpy_data, jax_data=None):
    # Create a figure with single plot
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Add title and labels
    ax.set_title('CartPole: Seed TD', fontsize=16)
    ax.set_ylabel('Throughput per Agent (agent/ms)')
    ...
    if numpy_data is not None:
        ax.plot(
            numpy_data['K'], 
            numpy_data['throughput_per_agent'],
            marker='o',
            linestyle='-',
        ...
        max_throughput_at_largest_k = numpy_data[numpy_data['K'] == largest_k]['throughput_per_agent'].values[0]
    ...
    if jax_data is not None:
        ...
        max_throughput_at_largest_k_jax = jax_data[jax_data['K'] == largest_k_jax]['throughput_per_agent'].values[0]
        
        if 'max_throughput_at_largest_k' in locals():
            if largest_k_jax >= largest_k and max_throughput_at_largest_k_jax > max_throughput_at_largest_k:
                max_throughput_at_largest_k = max_throughput_at_largest_k_jax
                largest_k = largest_k_jax
                best_frameworks = ['JAX']
            elif largest_k_jax == largest_k and max_throughput_at_largest_k_jax == max_throughput_at_largest_k:
    ...
    if torch_data is not None:
        ax.plot(
            torch_data['K'],
            torch_data['throughput_per_agent'],
            marker='o',
            linestyle='-',
        ...
        max_throughput_at_largest_k_torch = torch_data[torch_data['K'] == largest_k_torch]['throughput_per_agent'].values[0]
        
        if 'max_throughput_at_largest_k' in locals():
            if largest_k_torch >= largest_k and max_throughput_at_largest_k_torch > max_throughput_at_largest_k:
                max_throughput_at_largest_k = max_throughput_at_largest_k_torch
                largest_k = largest_k_torch
                best_frameworks = ['Torch']
            elif largest_k_torch == largest_k and max_throughput_at_largest_k_torch == max_throughput_at_largest_k:
                best_frameworks.append('Torch')
    
    # Add best framework annotation in BOTTOM right corner with matching style (since higher is better)
    ...
    plt.savefig(output_dir / "cartpole_throughput_per_agent.png", dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    if torch_data is not None or numpy_data is not None or jax_data is not None:
        create_throughput_subplot(torch_data, numpy_data, jax_data)
    else:
