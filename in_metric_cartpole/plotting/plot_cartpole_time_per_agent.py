...
# Function to create subplot for time_per_agent
def create_time_subplot(torch_data, numpy_data, jax_data=None):
    ...
    if numpy_data is not None:
        ...
        min_time_at_largest_k = numpy_data[numpy_data['K'] == largest_k]['time_per_agent'].values[0]
    ...
    if jax_data is not None:
        ...
        min_time_at_largest_k_jax = jax_data[jax_data['K'] == largest_k_jax]['time_per_agent'].values[0]
        
        if 'min_time_at_largest_k' in locals():
            if largest_k_jax >= largest_k and min_time_at_largest_k_jax < min_time_at_largest_k:
                min_time_at_largest_k = min_time_at_largest_k_jax
                largest_k = largest_k_jax
                best_frameworks = ['JAX']
            elif largest_k_jax == largest_k and min_time_at_largest_k_jax == min_time_at_largest_k:
    ...
    if torch_data is not None:
        ...
        min_time_at_largest_k_torch = torch_data[torch_data['K'] == largest_k_torch]['time_per_agent'].values[0]
        
        if 'min_time_at_largest_k' in locals():
            if largest_k_torch >= largest_k and min_time_at_largest_k_torch < min_time_at_largest_k:
                min_time_at_largest_k = min_time_at_largest_k_torch
                largest_k = largest_k_torch
                best_frameworks = ['Torch']
            elif largest_k_torch == largest_k and min_time_at_largest_k_torch == min_time_at_largest_k:
    ...
    plt.savefig(output_dir / "cartpole_time_per_agent.png", dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    if torch_data is not None or numpy_data is not None or jax_data is not None:
        create_time_subplot(torch_data, numpy_data, jax_data)
    else:
