...
def make_train_step(model):
    @jax.jit
    def train_step(params, prior_params, opt_state, batch):
        obs, act, rew, next_obs = batch
        def loss_fn(params):
...
def run_experiment(K=100, ensemble_size=30, num_seeds=1):
    ...
    for seed in range(num_seeds):
        ...
        train_step_fns = [make_train_step(model) for model, _ in models]  # pre-compile
            ...
            for agent_idx in range(K):
                ...
                if len(replay_buffer) % 48 == 0:
                    ...
                    for model_idx in range(ensemble_size):
                        model, params = models[model_idx]
                        prior_params = priors[model_idx]
                        opt_state = opt_states[model_idx]
                        train_step_fn = train_step_fns[model_idx]  # use precompiled fn
                        params, opt_state = train_step_fn(params, prior_params, opt_state, (obs_b, act_b, rew_b, next_obs_b))
                        models[model_idx] = (model, params)
                        opt_states[model_idx] = opt_state
